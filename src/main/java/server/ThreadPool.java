...
public class ThreadPool {
    ...
    public void sendEntriesReply(NodeConnectionInfo leaderId, NodeConnectionInfo nodeId, boolean success, int prevLogIndex, int prevLogTerm) {
        ExecutorService threadPool = Executors.newFixedThreadPool(1);
        threadPool.execute(new ReplierEntries(leaderId, nodeId, success, prevLogIndex, prevLogTerm));
    }
    ...
    private class ReplierEntries extends Worker implements Runnable {
        ...
        ReplierEntries(NodeConnectionInfo connectionId, NodeConnectionInfo sourceId, boolean success, int prevLogIndex, int prevLogTerm) {
            super(connectionId, -1, sourceId, prevLogIndex, prevLogTerm, null);
        ...
        public void run() {
            try {
                Registry registry = LocateRegistry.getRegistry(connectionId.getIpAddress(), connectionId.getPort());
                ServerInterface stub = (ServerInterface) registry.lookup(serviceName);
                stub.appendEntriesReply(myId, success, logIndex, logTerm);
            } catch (RemoteException e) {
                Debugger.log("Nao consegui ligar ao ip: " + connectionId.getIpAddress() + " com a porta: " + connectionId.getPort());
            } catch (NotBoundException e) {
                e.printStackTrace();
